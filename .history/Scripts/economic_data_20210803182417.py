...
def get_energy_price_data(sa):
    ...
    dfr.loc['Natural Gas Citygate Price $/Tcf ' + ia_note] = [inf_adj[i] * list(dfr.iloc[0]) for i in range(len(inf_adj))]
    dfr.loc['Natural Gas Residential Price $/Tcf ' + ia_note] = [inf_adj[i] * list(dfr.iloc[1]) for i in range(len(inf_adj))]
    dfr.loc['Natural Gas Commercial Price $/Tcf ' + ia_note] = [inf_adj[i] * list(dfr.iloc[2]) for i in range(len(inf_adj))]
    dfr.loc['Electricity Residential Price $/MMBtu ' + ia_note] = [inf_adj[i] * list(dfr.iloc[3]) for i in range(len(inf_adj))]
    dfr.loc['Electricity Commercial Price $/MMBtu ' + ia_note] = [inf_adj[i] * list(dfr.iloc[4]) for i in range(len(inf_adj))]
    return dfr