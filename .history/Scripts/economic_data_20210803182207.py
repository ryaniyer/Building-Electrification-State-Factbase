...
def get_energy_price_data(sa):
    ...
    elecresprice = list(dfr.iloc[2])
    eleccomprice = list(dfr.iloc[2])
    ia_note = '(Inflation Adjusted to ' + inflation_adjustment_year + ' USD)'
    dfr.loc['Natural Gas Citygate Price $/Tcf ' + ia_note] = [inf_adj[i] * citygate[i] for i in range(len(inf_adj))]
    dfr.loc['Natural Gas Residential Price $/Tcf ' + ia_note] = [inf_adj[i] * resprice[i] for i in range(len(inf_adj))]
    dfr.loc['Natural Gas Commercial Price $/Tcf ' + ia_note] = [inf_adj[i] * comprice[i] for i in range(len(inf_adj))]
    dfr.loc['Electricity Residential Price $/MMBtu' + ia_note] = [inf_adj[i] * resprice[i] for i in range(len(inf_adj))]
    dfr.loc['Electricity Commercial Price $/MMBtu' + ia_note] = [inf_adj[i] * comprice[i] for i in range(len(inf_adj))]
