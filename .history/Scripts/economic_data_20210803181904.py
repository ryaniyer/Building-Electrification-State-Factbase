...
def get_energy_price_data(sa):
    ...
    df.loc['Natural Gas Citygate Price ' + ia_note] = [inf2020[i] * citygate[i] for i in range(len(inf2020))]
    df.loc['Natural Gas Residential Price ' + ia_note] = [inf2020[i] * resprice[i] for i in range(len(inf2020))]
    df.loc['Natural Gas Commercial Price ' + ia_note] = [inf2020[i] * comprice[i] for i in range(len(inf2020))]
