...
def get_energy_price_data(sa):
    ...
    dfr.loc['Natural Gas Citygate Price ' + ia_note] = [inf_adj[i] * citygate[i] for i in range(len(inf_adj))]
    dfr.loc['Natural Gas Residential Price ' + ia_note] = [inf_adj[i] * resprice[i] for i in range(len(inf_adj))]
    dfr.loc['Natural Gas Commercial Price ' + ia_note] = [inf_adj[i] * comprice[i] for i in range(len(inf_adj))]
    dfr.loc['Electricity Residential Price ' + ia_note] = [inf_adj[i] * resprice[i] for i in range(len(inf_adj))]
    dfr.loc['Electricity Commercial Price ' + ia_note] = [inf_adj[i] * comprice[i] for i in range(len(inf_adj))]
