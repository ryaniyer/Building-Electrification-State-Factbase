...
for s in state_names:
    ...
    df_temp.T.to_csv(path+sa+'_EIA_SEDS_Data.csv')

    #EPA NEI Data Export:
    df_sector, df_type, df = epa_nei.get_EPA_NEI_Nonpoint_dfs(sa)
    with pd.ExcelWriter(path+'/Air_Quality/'+sa+'_EPA_Emissions_Data.xlsx') as writer:  
    ...
    with pd.ExcelWriter(path+sa+'/Air_Quality/'+sa+'_PM25_Racial_Disparities.xlsx') as writer:  
        df_sector.to_excel(writer, sheet_name='By Sector')
        df_type.to_excel(writer, sheet_name='By Pollutant')
        df.to_excel(writer, sheet_name='By County')
